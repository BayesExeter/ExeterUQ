...
for(i in 1:6) {
  ymin = tObs - 2*sqrt(tObsErr+Disc)
  ymax = tObs + 2*sqrt(tObsErr+Disc)
  plot(tData[, i], tData$SCMdata, xlab = cands[i], ylab = 'SCMdata', ylim = range(tData$SCMdata, ymin, ymax),
       pch = 20)
  abline(h=c(tObs, ymin, ymax ), col = 'blue', lty =2, lwd=c(1,3,3))
}
